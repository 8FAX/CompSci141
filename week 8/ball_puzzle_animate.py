...
def animate_move(stack_list, from_can, to_can):
    ...
    if type(stack_list) == list:
        canSize = [size(stack_list[0]), size(stack_list[1]), size(stack_list[2])]
    elif type(stack_list) == dict:
        canSize = []
        for can in stack_list.values():
            canSize.append(size(can))
        from_can = list(stack_list.keys()).index(from_can)
        to_can = list(stack_list.keys()).index(to_can)
